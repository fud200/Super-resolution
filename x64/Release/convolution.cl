...
__kernel void on_g_mat_1(__global float* dev_prefeature_map, __global float* dev_feature_map, __global float *mykernel, int H, int W) {
    //int t = blockIdx.x * blockDim.x + threadIdx.x;
    //int z = blockIdx.y * blockDim.y + threadIdx.y; 
    //printf("도냐");
    int t=get_global_id(0);
...
__kernel void on_g_trans(__global float* dev_Padding, __global float* dev_mykernel, __global float* dev_TempY, int OH, int OW, int PW, int k, int ksize, int y3) {
    //int t = blockIdx.x * blockDim.x + threadIdx.x;
    //int z = blockIdx.y * blockDim.y + threadIdx.y; 
    int t=get_global_id(0);
	int z=get_global_id(1);
